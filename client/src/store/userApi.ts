...
import { INotification2, IUser } from "../types/IUser";
...
export const userApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // getNotifications: builder.query<IUser[], { search: string, userId: string }>({
    //   query: ({ search, userId }) => `friendRequests?search=${search}&id=${userId}`,
    //   providesTags: (result) => {
    //     if (!Array.isArray(result)) {
    //       return [{ type: 'Users', id: 'LIST' }];
    ...
    notification: builder.mutation<IUser[], INotification2>({ //{type: 'addFriend', to: user._id, from: myUserId})
      query: (body) => ({
        url: 'notification',
        method: 'PATCH',
        body,
...
export const { useGetAllUsersByLoginQuery, useGetMyFriendsByLoginQuery, useGetUserQuery, usePatchFriendsMutation, useGetWaitFriendsQuery, useNotificationMutation } = userApi;
